...
const create = async() => {
  if (!form.name || !form.race || !form.gender || !form.charisma || !form.pace || !form.parry || !form.toughness || !form.hindrances || !form.edges) {
...
<template>
  <div class="col-md-6 offset-md-3">
    <form class="mb-3" @submit.prevent="create">
      <BaseInput v-model="form.discordID" label="Discord User ID:" type="text" class="mb-3" disabled hidden />
      <BaseInput v-model="form.name" label="Character Name:" type="text" class="mb-3" />
      ...
      <div class="mb-3">
        <div class="input-group">
          ...
          <button type="button" class="btn btn-outline-light" @click.prevent="addAttribute">Add</button>
        </div>
        <div v-if="attribute.items.length">
          <ul class="list-group">
            <li class="list-group-item border-light" style="background: none !important;" v-for="(item, index) in attribute.items" :key="index">
              <div class="input-group">
                ...
                <button type="button" class="btn btn-outline-warning" @click.prevent="updateAttribute(index)">Update</button>
                <button type="button" class="btn btn-outline-danger" @click.prevent="deleteAttribute(index)">Delete</button>
              </div>
            </li>
          </ul>
        </div>
      ...
      <div class="mb-3">
        <div class="input-group">
          ...
          <button type="button" class="btn btn-outline-light" @click.prevent="addSkill">Add</button>
        </div>
        <div v-if="skill.items.length">
          <ul class="list-group">
            <li class="list-group-item border-light" style="background: none !important;" v-for="(item, index) in skill.items" :key="index">
              <div class="input-group">
                ...
                <button type="button" class="btn btn-outline-warning" @click.prevent="updateSkill(index)">Update</button>
                <button type="button" class="btn btn-outline-danger" @click.prevent="deleteSkill(index)">Delete</button>
              </div>
            </li>
          </ul>
        </div>
